...
def test_tables_ui_is_served(client):
...
def test_profiles_endpoint_lists_columns(client):
    payload = client.get("/api/tables/profiles").json()
...
def test_extract_then_download(client, invoice_pdf, statement_pdf):
    ...
    download = client.get(f"/api/tables/download/{payload['job_id']}")
...
def test_non_pdf_upload_is_rejected(client):
    response = client.post(
        "/api/tables/extract", files=[("files", ("notes.txt", b"hello", "text/plain"))]
    )
...
def test_expired_job_gives_a_clear_error(client):
    assert client.get("/api/tables/download/deadbeef").status_code == 404
